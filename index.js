...
bot.on("guildMemberAdd", (member) => {
  ...
  welcomeChannel.send(
    "https://media.discordapp.net/attachments/685898272308461596/685898914351546388/SPOILER_unknown.png"
  );
});

bot.on("ready", () => {
  console.log(
    `Histy is up and running on ${bot.guilds.cache.size} servers as ${bot.user.tag}!`
  );
});
...
bot.on("messageCreate", (message) => {
  if (
    ...
    (!privateMode || message.author.id == "220737168857890816")
  ) {
    var msg = message.content.split(" ");
    let guild = message.member.guild;
    if (msg.length > 4 && msg.length < 32) {
      //4+ words, can either be say or say in general
      if (msg[0].toLowerCase() == "histy" && msg[1].toLowerCase() == "say") {
        if (msg[2].toLowerCase() == "in" && msg[3].toLowerCase() == "general") {
          //say in general
          msg[0] = " ";
          msg[1] = " ";
          msg[2] = " ";
...
bot.on("messageCreate", (message) => {
  if (
    (message.content.toLowerCase() == "private histy" ||
      message.content.toLowerCase() == "shut up histy") &&
    (message.author.id == "220737168857890816" ||
      message.author.id == "306801255274250243")
  ) {
    if (!privateMode) {
      privateMode = true;
    ...
    (message.content.toLowerCase() == "public histy" ||
      message.content.toLowerCase() == "ok histy") &&
    (message.author.id == "220737168857890816" ||
      message.author.id == "306801255274250243")
  ) {
    privateMode = false;
...
bot.on("messageCreate", (message) => {
  if (message.author.id == "683354722501001359") {
    ...
    for (var i = 0; i < msg.length; i++) {
      if (
        msg[i].toLowerCase() != lastBadWord &&
        badwords.includes(msg[i].toLowerCase())
      ) {
        lastBadWord = msg[i];
        message.channel.send(
          "Waaah!! <@220737168857890816> someone made me say a really bad word... *sob*"
        );
      } else if (
        msg[i].toLowerCase() == "love" ||
        msg[i].toLowerCase() == "luv"
      ) {
...
bot.on("messageCreate", (message) => {
  if (message.author.id == "220737168857890816") {
    if (
      message.content.toLowerCase() == "what word" &&
      lastBadWord != "nothing"
    ) {
...
bot.on("messageCreate", (message) => {
  switch (message.content.toLowerCase()) {
    ...
    case "die monster you dont belong in this world":
      message.channel.send(
        "It was not by my hand that i was given flesh once again. I was brought here by HUMANS who wish to pay me tribute."
      );
      break;
    ...
    case "i dont have it i have the strongest whip known to man":
      message.channel.send(
        "No you dont, I got mine day one you fucking fleaman."
      );
      break;
    ...
    case "i dont have fucking uno motherfucker i dont have two":
      message.channel.send(
        "Ritcher!! *throws glass* Go into an arcade and you will be able to download it for free,"
      );
      break;
    case "i dont have three i dont have fucking four five six":
      message.channel.send(
        "You know what? I dont give a fuck, its a fucking card game!"
      );
      break;
